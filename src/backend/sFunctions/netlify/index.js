...
class Netlify{
    ...
    getResponse = async (params) => {
        ...
        if(params.makeNewHatimArg === undefined){
            url = `http://localhost:8888/.netlify/functions/takeCuz${version}?key=${params.key}&subKey=${params.subKey}&cuzNo=${params.cuzNo}&name=${params.name}&alindi=${params.alindi}&ownerId=${params.ownerId}&dev=${params.dev}&writeTotalReadParts=${params.writeTotalReadParts}`;
        }else{
            url = `http://localhost:8888/.netlify/functions/takeCuz${version}?key=${params.key}&subKey=${params.subKey}&cuzNo=${params.cuzNo}&name=${params.name}&alindi=${params.alindi}&ownerId=${params.ownerId}&makeNewHatimArg=${params.makeNewHatimArg}&dev=${params.dev}&writeTotalReadParts=${params.writeTotalReadParts}`;   
        }
