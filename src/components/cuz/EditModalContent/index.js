...
import {
  editModalCuzlerActions,
  editModalCuzlerFunctions,
  useEditCuzlerModal,
} from '../../../features/editCuzlerModal';
import {useDispatch} from 'react-redux';
import {Loader} from 'tabler-icons-react';
import {useForm} from '@mantine/form';
import {
  cuzlerFunctionTriggerActions,
} from '../../../features/cuzlerFunctionTrigger';
...
export default function ContainedInputs (props) {
  ...
  return (
    <div style={{height: '100%'}}>
      <TextInput
        label={Language['/cuz'].CuzlerHatimCard.Modal.Header}
        value={form.values.header}
        onChange={e => form.setFieldValue ('header', e.currentTarget.value)}
        classNames={classes}
      />
      <Textarea
        mt="20px"
        label={Language['/cuz'].CuzlerHatimCard.Modal.Description}
      ...
      <Group position="center">
        {loading
          ...
          : <Button
              ...
              onClick={async () => {
                await dispatch (
                  editModalCuzlerFunctions.updateHatim ({
                    form,
                    dateValue,
                    subKey,
                  })
            ...
            >
              {Language['/cuz'].CuzlerHatimCard.Modal.Button}
            </Button>}
      </Group>
    </div>
  );
